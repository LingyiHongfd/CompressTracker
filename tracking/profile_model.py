...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='compresstracker', choices=['compresstracker'],
                        help='training script name')
    parser.add_argument('--config', type=str, default='compresstracker_vitb_256_4', help='yaml configure file name')
...
def evaluate_vit(model, template, search):
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, is_training='test')
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, is_training='test')
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    # Compute the Flops and Params of our CompressTracker
    args = parse_args()
    '''update cfg'''
    ...
    if args.script == "compresstracker":
        model_module = importlib.import_module('lib.models')
        model_constructor = model_module.build_compresstracker_test
